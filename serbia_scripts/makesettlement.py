...
config.read(r"/mnt/data/serbia.cfg")
csv_in_path = config.get('DEFAULT', 'csv_in')
csv_comp_path = config.get('DEFAULT', 'csv_comp')
img_path = config.get('DEFAULT', 'img')
csv_log_path = config.get('DEFAULT', 'csv_log')
lib_path = config.get('DEFAULT', 'lib')
import pandas as pd
import os
from itertools import product
import configparser

def get_csv_directory_from_config(config_file_path):
    config = configparser.ConfigParser()
    config.read(config_file_path)
    return config.get('DEFAULT', 'csv_in')

def get_library_path_from_config(config_file_path):
    config = configparser.ConfigParser()
    config.read(config_file_path)
    return config.get('DEFAULT', 'lib')
...
# Settings
config_file_path = '/home/pi/serbia/config/serbia.cfg'
csv_directory = get_csv_directory_from_config(config_file_path)
library_path = get_library_path_from_config(config_file_path)
excel_path = library_path
