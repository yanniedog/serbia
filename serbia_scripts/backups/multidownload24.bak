...
def print_exception(session_number, e):
    print_with_session_number(session_number, f"Exception: {str(e)}")
...
def update_status_and_save_csv(df, csv_path, index, status, session_number):
    ...
    gradient_color = get_gradient_color(settlement, religion, year, state, df)

    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]"
          f"(#{session_number}): "
          f"Row {index} "
          f"({gradient_color}{settlement}/{religion}/{year}/{state}{Style.RESET_ALL}/"
          f"{os.path.basename(url_parts.path)}) "
          f"{color}{status}{Style.RESET_ALL}")

def get_gradient_color(settlement, religion, year, state, df):
    if 'settlement' not in df.columns:
        return Style.RESET_ALL

    row = df.loc[(df['settlement'] == settlement) &
                 (df['religion'] == religion) &
                 (df['year'] == year) &
                 (df['state'] == state)].iloc[0]

    max_weight = max(row[f"{attr}_weight"] for attr in ['religion', 'year', 'state'])
    weight = row[f"{settlement}_weight"] * row[f"{religion}_weight"] * row[f"{year}_weight"] * row[f"{state}_weight"]
