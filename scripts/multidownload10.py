        ...
        try:
            ...
            for index, row in df.iterrows():
                if 'Status' in df.columns and not pd.isna(row['Status']):
                    print_with_session_number(session_number, f"Skipping URL {row['URL']} with status {row['Status']}")
                    continue
                ...
                if os.path.exists(local_image_path):
                    ...
                    update_status_and_save_csv(df, csv_path, index, 'E')
                    continue
                ...
                if check_image_with_session(image_url, session):
                    if download_image_with_session(image_url, local_image_path, session):
                        ...
                        update_status_and_save_csv(df, csv_path, index, 'D')
                    else:
                    ...
                    update_status_and_save_csv(df, csv_path, index, 'N')
            ...
            if restart_count < MAX_RESTARTS:
                ...
                restart_count += 1
                # save_state(STATE_FILE_PATH, csv_file, restart_count)  # commented out
...
if __name__ == "__main__":
    ...
    completed_successfully = False  # Initialize completed_successfully variable to False
    try:
        ...
        if browser:  # Check if browser is not None before calling quit()
            browser.quit()
