...
# Define the source directories and the aggregate directory
...
# Count the total number of CSV files in all source directories and the aggregate directory
total_csv_count = sum(len(list(Path(dir_path).glob("*.csv"))) for dir_path in source_dirs)
total_csv_count += len(list(Path(aggregate_dir).glob("*.csv")))

print(f"Total number of CSV files to redistribute: {total_csv_count}")
...
# Define the target directories for redistribution
