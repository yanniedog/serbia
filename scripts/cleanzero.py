...
def manage_empty_files(base_directory):
    log_directory = "/home/pi/serbia/logs/cleanup-log"
    ...
    if previous_exclusions:
        if input("Load the exclusion list from last time? (yes/no): ").strip().lower() == 'yes':
    ...
    backup_folder = "/home/pi/serbia/backup"
    if response == 'backup':
